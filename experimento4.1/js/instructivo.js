...
function inicializacion() {
    ...
    Array.from(ubicacionDestino).forEach(function (item) {
        if(item.dataset.tipo=="departamento")
            item.addEventListener("click", mostrar_localidades);
        else
...
function mostrar_localidades(event) {
    const departamentos = document.querySelectorAll('.departamentos');
    Array.from(departamentos).forEach(function (item) {
        item.style.display = "none";
    });

    const id_departamento = event.target.getAttribute("id");
    departamento_seleccionado = event.target.innerText;
    const localidades_departamento = document.querySelector(`[data-departamento='${id_departamento}']`);
    localidades_departamento.style.display = "";

    document.querySelector("#tipo-ubicacion").innerHTML = "la localidad en la";
}

function mostrar_demo_ubicacion(event) {
    resetear_divs();
    const ubicacion = event.target.getAttribute("id");
    const localidad_seleccionada = event.target.innerText;
    window.localStorage.setItem("ubicacion", ubicacion);
    window.localStorage.setItem("departamento_seleccionado", departamento_seleccionado);
    window.localStorage.setItem("localidad_seleccionada", localidad_seleccionada);
    window.location = "sufragio.html?ubicacion=" + ubicacion;
}
