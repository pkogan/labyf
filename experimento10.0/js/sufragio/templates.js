...
class Templates {
    ...
    crear_item_lista(boleta, normal) {
        ...
        template_data.candidatos[0].nombre === "Voto en Blanco" ?
        template_data.candidatos[0].sin_jefe_de_gob = {
            nombre:  "SIN CANDIDATO A JEFE DE GOB.",
            imagenes: ["default_interna"],
            partido: template_data.candidatos[1].partido
        }
             : (
                template_data.candidatos[0].nombre,
                template_data.candidatos[0].imagenes
            );
    ...
    __get_template_data_confirmacion(candidato, categoria, cantidad_categorias) {
        ...
        function boton_modificar_esta_activado(categoria, cantidad_categorias) {
            const es_modificar_en_lista_completa =
                constants.boton_modificar_en_lista_completa &&
                 this.__modo == "BTN_COMPLETA";
            const es_modificar_en_categorias =
        ...
        if (datos_candidato.constructor === Array) {
            ...
            if (datos_candidato.length > 1) {
                ...
                if (typeof (categoria.max_preferencias) !== "undefined") {
                    //cargar todas las preferencias, no solamente la primera
                } else {
    ...
    __construir_candidatos(candidato, campo, vista) {
        ...
        if(candidatos.length > 0){
            ...
            html_candidatos = template(data);
        }
        return html_candidatos
    }
