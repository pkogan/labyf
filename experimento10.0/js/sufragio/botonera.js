    ...
    if (aceptar_clicks) {
        ...
        if(en_menu_salida) {
            ...
            if (pagina_anterior !== null) {
                var modo = templateClass.get_modo();
                if (modo == "BTN_CATEG") {
...
function cargar_listas(boletas) {
    ...
    if (constants.agrupar_por_partido) {
        boletas = agrupar_boletas_por_partido(boletas);
    }
...
function agrupar_candidatos_por_partido(candidatos) {
    ...
    if (!constants.shuffle.por_sesion && constants.shuffle.internas) {
        partidos = shuffle(partidos);
    }
...
/**
 * Agrupa por partido las boletas. Tanto los partidos como las boletas aparecen al azar pero todas las boletas del mismo partido aparecen juntas.
 *
 * @param {*} boletas - Boletas que se van a agrupar por partidos.
 */
function agrupar_boletas_por_partido(boletas) {
    var boletas_ordenadas = [];
    var partidos = [];
    //Busco todos los partidos que hay
    for (var i in boletas) {
        let cod_partido = null;
        if (boletas[i].codigo === "BLC"){
            cod_partido = boletas[i].codigo;
        }else{
            cod_partido = boletas[i].lista.cod_partido;
        }
        if (partidos.indexOf(cod_partido) == -1) {
            partidos.push(cod_partido);
        }
    }
    ...
    for (var l in partidos) {
        for (var j in boletas) {
            let cod_partido = null;
            if (boletas[j].codigo == "BLC"){
                cod_partido = boletas[j].codigo;
            }else{
                cod_partido = boletas[j].lista.cod_partido;
            }
            if (partidos[l] === cod_partido) {
                boletas_ordenadas.push(boletas[j]);
            }
        }
    }

    return boletas_ordenadas;
}
