...
function cargar_listas(boletas) {
    ...
    for (var m in boletas) {
        ...
        let candidatos_boleta = get_candidatos_boleta(boleta);
        if (boleta.clase == "Candidato" || boleta.clase == "Blanco") {
            ...
            if (codigo_lista != constants.cod_lista_blanco) {
                let item = templateClass.crear_item_lista(boleta, candidatos_boleta, true);
                html += item;
            } else {
                blanco = 1;
            }
...
function cargar_listas_completas_segmentadas(boletas, preagrupada, hay_agrupaciones_municipales) {
    /* Carga las listas completas segmentadas */
    // if(!constants.shuffle.por_sesion && constants.shuffle.boletas){
        boletas = shuffle(boletas);
    // }
    console.log(constants.shuffle.por_sesion, constants.shuffle.boletas);
    ...
    let html = "";
    let blanco = 0;
    for(let m in boletas){
        let boleta = boletas[m];
        let lista = local_data.agrupaciones.one({"codigo": boleta.id_lista});
        boleta.lista = lista;
        ...
        if(boleta.clase == "Candidato" || boleta.clase == "Blanco"){
            ...
            let codigo_lista = boletas[m].id_lista;
            if(codigo_lista != constants.cod_lista_blanco){
                let item = templateClass.crear_item_lista(boleta, true);
                html += item;
            } else {
                blanco = 1;
            }
    ...
    pantalla.only();
    document.querySelector(pantalla.id).classList.value = "";
    var clase_listas = templateClass.get_template_candidatos(boletas.length - blanco);
    document
        .querySelector(pantalla.id)
        .classList.add("pantalla", "opciones", "sinbarra");
    document
        .querySelector(pantalla.id)
        .classList.add(clase_listas);
    ...
    if(_votando){
        ...
        if (hay_agrupaciones_municipales) {
            ...
            document.querySelector("#agrupaciones_municipales").onclick = click_lista;
            document.querySelector("#agrupaciones_municipales").classList.remove("seleccionado");
        }
...
function cargar_candidatos(data) {
    ...
    categoria_modificada = cambio_tittle_cod_dato(data.categoria)
    cambiar_categoria(data.categoria);
    contenido_solapa(categoria_modificada.nombre);
...
/** Cambia el contenido de la solapa del titulo dependiendo
 * de los cod_datos agregados dentro del array. 
 * 
 * * @param {*} categorias - Categorías a mostrar.
*/
function cambio_tittle_cod_dato(categoria) {
    let ubicaciones_selectas = [
        "NE.14.15",
        "NE.14.15.6.EXT",
        "NE.3.29",
        "NE.3.29.4.EXT",
    ...
    let c = categoria;
    if (ubicaciones_selectas.includes(constants.cod_datos)) {
        if (categoria.nombre === "Intendente/a") {
            c = {
                ...categoria,
                nombre: "Intendente/a y Vice"
            }
        }
    }
    return c;
