...
var _ultima_selec_cod_lista = null;
...
function _cargar_pantalla_lista_completa(cod_partido) {
    ...
    if (typeof constants.mesa_ext_no_agrupar_cargo !== "undefined" && typeof constants.cod_datos !== 'undefined' &&
    constants.mesa_ext_no_agrupar_cargo && constants.cod_datos.includes('EXT')) {
        no_agrupar_cargo_extranjera = true
    }
    // Armamos el filtro por defecto con el que vamos a filtrar las listas.

    var agrupar_cargo = local_data.categorias.first().codigo;
    ...
    if (typeof cod_partido !== "undefined") {
        ...
        // Agrego la condicion que corresponde al maximo cargo seleccionado, en este caso (GOB)
        if (_ultima_selec_cod_lista != null && no_agrupar_cargo_extranjera ? false : constants.agrupar_cargo) {
        ...
        data = boletas;
    } else if (no_agrupar_cargo_extranjera ? false : constants.agrupar_cargo) {
    ...
    if (data.length == 1 && constants.seleccionar_lista_unica) {
        ...
        if (constants.asistida) {
            cargar_listas_asistida(
                data,
                no_agrupar_cargo_extranjera ? false : constants.agrupar_cargo,
                hay_agrupaciones_municipales
            );
        } else {
            cargar_listas(
                data,
...
function _cargar_candidatos_categoria(categoria, agrupacion) {
    ...
    if(categoria.max_preferencias!=="undefined"){
        ...
        var listas = local_data.agrupaciones.many({clase: 'Lista'});
        var candidatos = [];
    ...
    if ((typeof(categoria.max_preferencias) !== "undefined")
        ...
        if (preferencias_tmp == null) {
            ...
            if (seleccionar_preferente_unico) {
                _seleccion.preferencias[categoria.codigo] = [];
                const fake_dom_element = $("<div></div>");
...
function seleccionar_lista(cod_lista) {
    ...
    if (typeof boleta === "undefined") {
        ...
        if (cod_lista == constants.cod_lista_blanco) {
            ...
            var cod_categoria = local_data.categorias.first().codigo;
            ...
            // Intento guardar el codigo de lista del GOB para poder agregar este filtro cuando se colapsa
            // por partido.
            ...
            if (typeof candidato == "undefined") {
                ...
                if (_ultima_selec_cod_lista == null) {
                    _ultima_selec_cod_lista = candidato.id_umv;
                }
            }
            ...
            for (var idx in data) {
                ...
                if (cod_alianza) {
                    ...
                    alianza = local_data.agrupaciones.many(filter)
                    data[idx]['alianza'] = alianza[0]
                }
                if (typeof constants.no_mostrar_lista_cod != "undefined") {
                    if (!constants.no_mostrar_lista_cod.includes(data[idx].codigo)) {
            ...
            if (data.length == 1 && constants.seleccionar_lista_unica) {
                ...
                if (constants.asistida) {
                    ...
                    let hay_frentes = (typeof constants.candidato_muestra_frente != "undefined" &&
                                        constants.candidato_muestra_frente.includes(cod_lista));
                    cargar_listas(data, true, false, hay_frentes);
                    if (typeof candidato !== "undefined") {
                    const cargo = candidato['categoria']['nombre']
                    let candidato_modificado = JSON.parse(JSON.stringify(candidato));
                    candidato_modificado['categoria']['nombre'] = (cargo === "Gobernador y Vice") ? 'Gobernador' : cargo
...
function get_lista_candidato(candidato) {
    let lista = local_data.agrupaciones.one({ id_candidatura: candidato.cod_lista });
    if(candidato.dni==="26701393"){
        return {
            ...lista,
            imagenes: ["3733.1869.1869"]
        }
    }
    return lista; 
}
