...
class Templates {
    ...
    crear_item_lista(boleta, normal, preagrupada) {
        ...
        template_data.cinco_candidatos = candidatos.length === 5 ? true : false;
        template_data.candidato_muestra_frente = false
        if (typeof constants.candidato_muestra_frente != "undefined" &&
        constants.candidato_muestra_frente.includes(candidatos[0].id_candidatura) ) {
            template_data.candidato_muestra_frente = boleta.alianza ? true : false
        }
        template_data.mesa_ext_no_agrupar_cargo = false;
        ...
        constants.mesa_ext_no_agrupar_cargo && constants.cod_datos.includes('EXT')) {
            template_data.mesa_ext_no_agrupar_cargo = true;
        }
        let template_name = this.__get_template_name_lista(normal)
        var item = tarjeta_seleccion_lista_html(template_data, template_name);
        return item;
    
        function tarjeta_seleccion_lista_html(template_data, template_name) {
    ...
    __get_template_name_lista(normal) {
        return normal ? "lista" : "lista_agrupacion_candidatos";
    }
    /**
    ...
    __get_template_data_confirmacion(candidato, categoria, cantidad_categorias) {
        ...
        template_data.nombre_partido = template_data.blanco
            ? ""
            : nombre_partido;
        template_data.vice_gobernador = template_data.categoria.codigo === "GOB" ? true : false;
        return template_data;
    }
    ...
    __get_template_data_verificacion(candidato, categoria) {
        ...
        template_data.muestra_secundarios = categoria.codigo == 'GOB' ? true : false;
        return template_data;
    }
